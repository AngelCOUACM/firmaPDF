...
if($_SERVER['REQUEST_METHOD'] == 'POST'){/*Inicia if*/
	...
	if($user == null && $pass == null){/*Inicia if*/
		...
		$sel = $con->query("SELECT nombre, profesion, telefono, direccion, correo,  nivel, area, foto, bloqueo, password FROM usuarios WHERE correo='$usuario' AND password='$pass2' AND bloqueo=1 ");
		...
		if($correo == $usuario && $contra == $pass2 && $nivel == 'ADMINISTRADOR' ){//Inicia if
		...
		elseif($correo == $usuario && $contra == $pass2 && $nivel == 'ALUMNO' ){//Inicia elseif
			$_SESSION['nombre'] = $nombre;
			$_SESSION['matricula'] = $matricula;
			$_SESSION['colegio'] = $colegio;
			$_SESSION['telefono'] = $telefono;
			...
			header('location:../extend/alerta.php?msj=Usuario o contrase̱a incorrectos&c=salir&p=salir&t=error');
		}//Termina else
